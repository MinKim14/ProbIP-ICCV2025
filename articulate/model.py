...
class ParametricModel:
    ...
    def forward_kinematics(
        ...
        pose = pose.reshape(pose.shape[0], -1, 3, 3)
        j, v = [
        ...
        T_global[..., -1:] -= torch.matmul(
            T_global, M.append_zero(j, dim=-1).unsqueeze(-1)
        )
        ...
        vertex_global = torch.matmul(
            T_vertex, M.append_one(v, dim=-1).unsqueeze(-1)
        ).squeeze(-1)[..., :3]
    ...
    def view_joint(self, joint_list: list, fps=60, distance_between_subjects=0.8):
        ...
        joint_list = [
            (j.view(-1, len(self.parent), 3) - j.view(-1, len(self.parent), 3)[:1, :1])
            .cpu()
            .numpy()
            for j in joint_list
        ]
        ...
        for i in range(len(joint_list)):
            v = np.stack(
                [vc.joints_to_mesh(frame, self.parent)[0] for frame in joint_list[i]]
            )
    ...
    def view_mesh(
        ...
        :param fps: Sequence FPS.
        :param distance_between_subjects: Distance in meters between subjects. 0.2 for hand and 0.8 for body is good.
        :param colors: List of colors for each subject in RGB format (0-1 range). If None, uses default colors.
        """
        import vctoolkit.viso3d as vo3d
        ...
        num_vertices = verts.shape[1]
        frame_colors = np.zeros((num_frames, num_vertices, 3))
        
        start_idx = 0
        for i in range(len(vertex_list)):
            color = colors[i % len(colors)]
            num_verts = vertex_list[i].shape[1]
            frame_colors[:, start_idx:start_idx + num_verts] = color
            start_idx += num_verts

        if verts.shape[0] > 1:
            vo3d.render_sequence_3d(
                verts,
                faces,
                720,
                720,
            ...
            vo3d.vis_mesh(verts[0], faces, vertex_colors=frame_colors[0])
