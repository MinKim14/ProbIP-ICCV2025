...
__all__ = [
    "transformation_matrix_np",
    "adjoint_transformation_matrix_np",
    "transformation_matrix",
    "decode_transformation_matrix",
    "inverse_transformation_matrix",
    "bone_vector_to_joint_position",
    "joint_position_to_bone_vector",
    "forward_kinematics_R",
    "inverse_kinematics_R",
...
def transformation_matrix(R: torch.Tensor, p: torch.Tensor):
    ...
    OI = torch.cat(
        (
            torch.zeros(list(Rp.shape[:-2]) + [1, 3], device=R.device),
            torch.ones(list(Rp.shape[:-2]) + [1, 1], device=R.device),
        ),
        dim=-1,
    )
